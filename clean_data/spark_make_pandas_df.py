...
if __name__ == "__main__":
    ...
    temp_df.write.csv(outputdir+"processed_twitter_train_data_1.csv")
    print("Create Train DF 2")
    dates = ("2018-12-25",  "2019-01-01")
    date_from, date_to = [f.to_date(f.lit(s)).cast(TimestampType()) for s in dates]
    temp_df = df.where((df.date_col > date_from) & (df.date_col < date_to))
    temp_df.write.csv(outputdir+"processed_twitter_train_data_2.csv")
    print("Create Test DF 1")
    dates = ("2018-12-31",  "2019-01-13")
    date_from, date_to = [f.to_date(f.lit(s)).cast(TimestampType()) for s in dates]
    temp_df = df.where((df.date_col > date_from) & (df.date_col < date_to))
    temp_df.write.csv(outputdir+"processed_twitter_test_data_1.csv")
    print("Create Test DF 1")
    dates = ("2019-01-12",  "2019-01-23")
    date_from, date_to = [f.to_date(f.lit(s)).cast(TimestampType()) for s in dates]
    temp_df = df.where((df.date_col > date_from) & (df.date_col < date_to))
    temp_df.write.csv(outputdir+"processed_twitter_test_data_2.csv")
    print("Create Val DF")
    dates = ("2019-01-22",  "2019-01-25")
    date_from, date_to = [f.to_date(f.lit(s)).cast(TimestampType()) for s in dates]
    temp_df = df.where((df.date_col > date_from) & (df.date_col < date_to))
    temp_df.write.csv(outputdir+"processed_twitter_val_data.csv")
    sc.stop()
